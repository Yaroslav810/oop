...
%PROGRAM% decrypt empty.txt %OUT% -1 > nul && goto err
%PROGRAM% decrypt empty.txt %OUT% 256 > nul && goto err
echo Test #4: passed!
...
rem Crypt exe
%PROGRAM% crypt "C:\Volgatech\second_course\oop\oop\cmake-build-debug\labs\lab1\Crypt\crypt.exe" %OUT% 10 || goto err
%PROGRAM% decrypt %OUT% "crypt_exe.txt" 10 || goto err
fc "crypt_exe.txt" "C:\Volgatech\second_course\oop\oop\cmake-build-debug\labs\lab1\Crypt\crypt.exe" > nul || goto err
echo Test #8: passed!
